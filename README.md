# 宝宝照护记录

手机和平板优先的家庭共享照护 APP，用于记录母乳、奶粉、AD、VD、益生菌、推拿和排便等照护活动。

## 本地校验

```bash
npm run check
```

该命令依次执行单元测试、静态检查（Biome lint）、前后端严格类型检查、生产构建和 CSS 结构检查。疫苗建议计划与照护周期逻辑由 `shared/` 下的跨端模块统一提供。

## 主要功能

### 照护记录
- 今日奶量、喂奶次数、排便次数和最近喂奶时间
- 母乳与奶粉分开记录，支持自定义喂奶类型组合
- AD、VD、益生菌、推拿等项目一键记录
- 排便时间及大、中、小
- 历史日期浏览与搜索

### 数据统计
- 七日、月度和总数据趋势；月度每周奶量按周一至周日统计
- 每周身高体重记录，首页未记录时提醒，完成后收入宝宝档案
- 档案显示宝宝月龄、最新身高体重、历史变化和录入人

### AI 智能
- 配置兼容接口的 AI 模型后，每日自动生成前一天的「宝宝日报」
- 未配置 AI 时保留基础日报，并明确显示当前状态
- 当前不提供语音录入、语音转写或自然语言转记录功能

### 家庭协作
- 爸爸、妈妈、爷爷、奶奶独立身份登录
- 爸爸固定为超管，可将妈妈、爷爷和奶奶设为管理员或普通用户
- 每条记录保存创建人和最后修改人
- 家庭成员新增或修改记录后，其他设备实时更新
- 家庭设备离线暂存和恢复联网后的自动同步

### 宝宝资料
- 宝宝昵称（如「小糯米」）显示在首页问候语中
- 自定义称谓（爸爸/妈妈/爷爷/奶奶），首页自动根据登录身份展示
- 头像上传：支持本地裁剪（圆形裁剪框 + 缩放），服务端自动压缩为 WebP 格式

### 消息推送
- 通过 PushPlus 推送到普通微信，无需企业微信
- 早间日报：每日定时发送前一天的照护总结
- 喂奶间隔提醒：两级阈值（轻度 🟡 / 重点 🔴）
- 用药与照护提醒：定时提醒未完成的照护项目
- 独立开关：每种推送类型可单独启用/禁用
- 测试按钮：可单独测试每种推送类型是否正常工作

### 数据管理
- 每 6 小时自动保存服务器备份，并支持手动备份、导出与事务恢复
- 管理员可软删除、查看已删除记录、恢复或二次确认后彻底删除
- 同日同种用药重复保护

### 界面体验
- 首页、记录页和档案页支持移动端下拉刷新
- 实时连接不可用时自动低频检查
- 桌面端、平板和移动端自适应布局
- Android 状态栏和系统导航栏适配浅色/深色系统主题

## Web 与 Android App

网页是唯一业务实现，`android-app/` 是与网页部署并行的原生 Android 壳。App 通过 WebView 加载所选服务器，不复制前端、API 或数据库，因此 Web 和 App 共用同一套业务能力与服务端数据。

- App 可在“设置 → 服务器环境”切换局域网与外网地址。
- Android 1.3.0 起可在“设置 → APP 通知”分别开启早报、喂奶、照护和疫苗提醒；前三类约每 15 分钟同步，疫苗预约在本地提醒并可添加到系统日历。
- 同一个服务器地址会保持登录 Cookie；不同域名、协议或端口属于不同来源，切换后可能需要分别登录。
- 服务器部署新版本后，网页刷新或重新打开 App 通常即可加载新版；已在前台打开的页面不会自动替换整套前端代码。
- 原生壳、图标、权限或 Android 配置变化时，必须提高版本号、重新构建并安装 APK。

Android 构建与使用说明见 [`android-app/README.md`](./android-app/README.md)，完整文档导航见 [`docs/README.md`](./docs/README.md)。

## Unraid 部署

项目统一使用 `babycare-website` 作为包名、镜像名、Compose 服务名和容器名。默认目录如下，不会创建额外的 Unraid 共享目录：

```text
/mnt/user/appdata/baby-care/
├── app/       # GitHub 源码
├── data/      # SQLite 数据库
└── backups/   # 自动备份
```

### 首次部署

```bash
mkdir -p /mnt/user/appdata/baby-care
cd /mnt/user/appdata/baby-care
git clone --branch main https://github.com/x1ngodlike/babycare-website.git app
cd app
chmod +x babycare.sh
./babycare.sh
```

脚本显示全中文菜单：

```text
1. 首次部署或重新构建
2. 更新到 GitHub 最新版本
3. 备份数据
4. 查看运行状态
5. 查看实时日志
6. 停止服务
7. 启动服务
0. 退出
```

也可以直接使用英文命令：

```bash
./babycare.sh deploy
./babycare.sh update
./babycare.sh backup
./babycare.sh status
./babycare.sh logs
./babycare.sh stop
./babycare.sh start
```

“更新”会依次备份数据库、拉取 GitHub 最新代码、删除本项目以前创建的新旧容器、构建镜像并创建唯一的 `babycare-website` 容器。新容器通过完整健康检查后，脚本会精确删除本项目上一版且已不再被使用的镜像，不会全局清理其他 Unraid 应用的镜像或缓存。脚本不会删除 `/mnt/user/appdata/baby-care/data`。

每次备份都会短暂停止正在运行的本项目容器，完整保存数据库目录、`.env` 和 Compose 配置，然后恢复原运行状态。备份固定保存在：

```text
/mnt/user/appdata/baby-care/backups
```

首次生成的 `.env` 使用爸爸密码 `qwe123`，妈妈、爷爷、奶奶密码 `111111`，并生成随机会话密钥。部署成功后建议修改密码，再执行 `./babycare.sh deploy`。

完整健康检查同时验证容器中的 `/app/dist/index.html`、网页首页和后端接口，避免只有后端启动而前端文件缺失。

### 恢复备份

先停止服务，并将当前数据目录改名保留。以下备份文件名需要替换为实际文件：

```bash
cd /mnt/user/appdata/baby-care/app
./babycare.sh stop
mkdir -p /mnt/user/appdata/baby-care/restore-temp
tar -xzf /mnt/user/appdata/baby-care/backups/babycare-website-日期时间.tar.gz \
  -C /mnt/user/appdata/baby-care/restore-temp
mv /mnt/user/appdata/baby-care/data \
  /mnt/user/appdata/baby-care/data.before-restore
mv /mnt/user/appdata/baby-care/restore-temp/data \
  /mnt/user/appdata/baby-care/data
cp /mnt/user/appdata/baby-care/restore-temp/.env .env
chown -R 1000:1000 /mnt/user/appdata/baby-care/data
./babycare.sh deploy
```

确认恢复后的记录无误，再自行处理 `data.before-restore`；它是恢复前的数据副本。

SQLite 数据库存放在 `data/baby-care.db`。应用每 6 小时把完整 JSON 自动保存到 `data/backups/`，最多保留最近 30 份；Unraid 中对应 `/mnt/user/appdata/baby-care/data/backups/`，不会建立新的共享目录。爸爸可在设置页立即备份、选择服务器备份进行完整恢复，或手动导出和导入备份文件；服务器恢复前会先自动保存当前数据，恢复后宝宝资料、照护记录和操作历史与所选备份完全一致。不要在容器运行时只复制单个数据库文件，以免遗漏 WAL 中尚未合并的记录。

## AI 模型配置

宝宝日报的智能总结依赖 AI 模型，在应用内“设置 → AI 模型设置”填写兼容接口地址与 API 密钥即可，无需在 `.env` 中配置。配置完成后，服务端每天北京时间 00:00 自动生成前一天的「宝宝日报」。

爸爸登录后可以在“设置 → AI 模型设置”中填写 API 密钥。默认配置为：

```text
接口地址：https://api.deepseek.com
模型名称：deepseek-v4-flash
```

模型仅用于生成宝宝日报摘要和建议。密钥保存在服务器 SQLite 数据库中，设置接口只返回是否已经配置和末四位提示，不会把完整密钥发送到网页。普通用户可以查看生成结果，但不能查看、测试或修改模型配置。

正式外网访问建议通过 Nginx Proxy Manager、Tailscale 或 Cloudflare Tunnel 配置 HTTPS，并将 `.env` 中 `COOKIE_SECURE` 改为 `true`。不要把 3000 端口直接暴露到公网。

## 消息推送配置

消息推送使用 PushPlus 服务，可以将提醒发送到普通微信，无需安装企业微信 APP。

### 配置步骤

1. 访问 [pushplus.plus](https://www.pushplus.plus) 扫码登录
2. 复制你的用户 Token
3. 如需家庭成员同时接收，进入「一对多消息」新建 Topic，让家人扫码加入
4. 在应用内「设置 → 消息推送」中填写 Token 和 Topic
5. 启用推送开关，点击保存

### 推送类型

| 类型 | 说明 | 默认时间/阈值 |
|------|------|--------------|
| 早间日报 | 发送前一天的照护总结、用药计划、疫苗安排 | 每日 08:00 |
| 喂奶间隔（轻度） | 距上次喂奶超过设定时间时提醒 | 150 分钟 |
| 喂奶间隔（重点） | 距上次喂奶严重超时，带宝宝饥饿提示 | 180 分钟 |
| 用药与照护 | 定时提醒未完成的照护项目 | 按项目设置的提醒时间 |

### 注意事项

- 推送配置保存在服务器 SQLite 数据库中，Token 只显示末四位
- 每种推送类型有独立开关，可单独启用/禁用
- 提供测试按钮，可单独验证每种推送是否正常工作
- 仅超管（爸爸）可配置推送设置

## 本地开发

```bash
npm install
npm run dev
```

- APP：`http://localhost:5173`
- API：`http://localhost:3000`
- 默认密码：爸爸 `qwe123`；妈妈、爷爷、奶奶均为 `111111`

## 家庭身份与权限

- 爸爸：超管，可在“档案”修改宝宝资料，并在点击顶部身份区后进入系统设置，管理家庭权限、用药项目、AI 模型和备份。
- 妈妈、爷爷、奶奶：由超管设置为管理员或普通用户。管理员可管理用药项目和回收站；普通用户可查看、新增和修改照护记录。
- 创建和修改记录时，服务器根据登录会话自动写入操作人，网页不能伪造。
- 升级前的既有记录统一标记为“历史数据”。

## 性能与刷新优化

- 实时刷新按事件作用域定向：新增记录、修改资料只重载对应数据切片（`records` / `profile`），不再每次触发全部 9 类数据全量下载。
- 首页仅加载当日记录即可使用；进入趋势或历史页时才懒加载全量记录。刷新范围跟随当前所在页面（今日页取当日，趋势 / 历史页取全量），因此打开过趋势页后首页仍保持轻量。
- 趋势页进入时一次性按日聚合，避免每次渲染重复过滤全量记录。
- 后端 Express 已设置 `trust proxy`，在 Nginx Proxy Manager / Cloudflare 等反代后取真实客户端 IP，登录失败限流不再全家人共用一个桶。
- 修复历史 / 趋势页数据不停闪烁：此前刷新函数依赖随页面切换变化，触发加载链反复重跑；改为读取当前页引用，函数身份恒定。

## 检查与构建

```bash
npm run check
```

该命令依次运行单元测试、静态检查（`npm run lint`，Biome）、前后端严格类型检查、生产构建和 CSS 结构检查。需要单独排查时，也可分别运行 `npm test`、`npm run lint`、`npm run typecheck`、`npm run build` 和 `npm run check:css`。

### 图片维护工具

新增或替换批量图片后，可先执行只读检查：

```bash
python3 -m pip install Pillow
npm run images:check
```

确认目标与尺寸无误后，再运行 `npm run images:optimize`。写入前会把所有目标文件备份到 `public/.image-backup-日期时间/`；PWA 的 `icon-192.png` 和 `icon-512.png` 固定跳过。该工具不属于日常构建流程。

## 数据与隐私

- 当前没有语音录入或语音转写，应用也不申请麦克风权限。
- 配置 AI 后，日报所需的结构化照护数据会发送给所选择的 AI 服务生成摘要和建议。
- 应用没有公开注册入口，适合作为家庭内部工具。
- 服务器自动备份与数据库位于同一份 Unraid 存储，建议定期把导出的 JSON 文件另存到其他设备。

## 历史打印版

重构前的静态打印项目保存在 APP 仓库外的 `../print-schedule/`，不会进入 Git、Docker 镜像或生产构建。
