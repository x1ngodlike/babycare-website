...
export function getAiFeedingInsights(): AiFeedingInsights | null {
  const row = db.prepare('SELECT summary, insights_json AS insightsJson, alert, gap_minutes AS gapMinutes, next_feed_at AS nextFeedAt, records_hash AS recordsHash, created_at AS createdAt, updated_at AS updatedAt FROM ai_feeding_insights WHERE id = 1').get();
  if (!row) return null;
  return {
    summary: row.summary,
