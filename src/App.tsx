...
function DailyReport({ capabilities, online, onOpenSettings, superadmin, userId, allowAutoOpen }: { capabilities: Capabilities; online: boolean; onOpenSettings(): void; superadmin: boolean; userId: FamilyId; allowAutoOpen: boolean }) {
  ...
  return (
    <section className="daily-report info-summary-static" aria-label="昨日报告"><div><b>昨日报告</b><small>{loading ? '正在读取…' : error || (!online ? '联网后可查看' : !capabilities.aiEnabled ? '尚未配置 AI 模型' : superadmin ? '报告尚未生成' : '报告还没准备好')}</small></div>{!loading && superadmin && online && (!capabilities.aiEnabled ? <button className="text-button" onClick={onOpenSettings}>去设置</button> : <button className="text-button" disabled={busy} onClick={generate}>{busy ? '生成中…' : '生成报告'}</button>)}</section>
  );
}

function TodayView({ profile, records, vaccineRecords, vaccineCatalog, vaccineRemindersEnabled, careItems, capabilities, online, onOpenSettings, onCompleteVaccine, onAppointmentVaccine, manager, superadmin, userId, allowReportAutoOpen, weeklyGrowth, onAddGrowth, onAdd, onSupplement, onEdit, onDelete, onAudit }: { profile: Profile; records: CareRecord[]; vaccineRecords: VaccineRecord[]; vaccineCatalog: VaccineCatalogItem[]; vaccineRemindersEnabled: boolean; careItems: CareItem[]; capabilities: Capabilities; online: boolean; onOpenSettings(): void; onCompleteVaccine(item: VaccinePlanItem): void; onAppointmentVaccine(item: VaccinePlanItem): void; manager: boolean; superadmin: boolean; userId: FamilyId; allowReportAutoOpen: boolean; weeklyGrowth?: GrowthRecord; onAddGrowth(): void; onAdd(type: RecordType): void; onSupplement(value: Supplement): Promise<void>; onEdit(record: CareRecord): void; onDelete(record: CareRecord): void; onAudit(record: CareRecord): void }) {
  ...
  return <div className="today-layout">
    <div className="today-workbench">
      <section className="baby-hero"><div><p className="kicker">今日 · {new Date().toLocaleDateString('zh-CN', { month: 'long', day: 'numeric', weekday: 'long' })}</p><div className="baby-title-line"><h1>{profile.name}</h1><span>{calculateAge(profile.birthDate)}</span></div><div className="hero-status"><p>{lastFeed ? `上次喂奶 ${new Date(lastFeed.occurredAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false })}` : '今日还未喂奶'}</p>{latestRecord && <p>最近记录 {new Date(latestRecord.occurredAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false })} · {summary(latestRecord)}</p>}</div></div><img src="/bear-bottle.png" alt="" /></section>
