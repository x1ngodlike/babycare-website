...
import { Baby, Bell, Bot, LogOut, Monitor, Pill, RefreshCw, Save, Search, Send, Server, Syringe, Users } from 'lucide-react';
import { api, ApiError, type GrowthAssessment, type GrowthIndicatorAssessment } from './api';
...
const typeIcons: Record<RecordType, string> = { feeding: '/icons/quick-feeding.png', supplement: '/icons/record-care.png', bowel: '/icons/quick-bowel.png', note: '/icons/quick-note.png' };
...
const careItemIconSources: Record<CareItemIcon, string> = {
  medicine: '/icons/record-medicine.png', massage: '/icons/record-massage.png', bath: '/icons/record-bath.png', care: '/icons/record-care.png'
};
const careItemFor = (name: string | null | undefined, items: CareItem[]) => items.find(item => item.name === name);
const careItemCategory = (name: string | null | undefined, items: CareItem[]): CareItemCategory => careItemFor(name, items)?.category || (name === '推拿' ? 'care' : 'medication');
const careItemIcon = (value: CareRecord | DraftRecord, items: CareItem[]) => value.type === 'supplement' ? careItemIconSources[careItemFor(value.supplement, items)?.icon || (value.supplement === '推拿' ? 'massage' : 'medicine')] : typeIcons[value.type];
const selectableCareItems = (items: CareItem[], current?: string | null) => items.filter(item => item.active || item.name === current);
...
function CareItemEditor({ item, nextOrder, onClose, onSaved }: { item?: CareItem; nextOrder: number; onClose(): void; onSaved(item: CareItem): void }) {
  ...
  const initial = { name: item?.name || '', category: item?.category || 'medication' as CareItemCategory, icon: item?.icon || 'medicine' as CareItemIcon, scheduleType: item?.scheduleType || 'as_needed' as CareItem['scheduleType'], intervalDays: item?.intervalDays || 2, scheduleStartDate: item?.scheduleStartDate || defaultStart, reminderTime: item?.reminderTime || '', scheduleEndDate: item?.scheduleEndDate || '' };
...
function careItemHomeStatus(item: CareItem) {
  ...
  if (item.scheduleType === 'as_needed') return '按需记录 · 手动添加';
