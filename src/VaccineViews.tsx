...
export function VaccineReminderCard({ profile, records, catalog, onComplete, onAppointment }: { profile: Profile; records: VaccineRecord[]; catalog: VaccineCatalogItem[]; onComplete(item: VaccinePlanItem): void; onAppointment(item: VaccinePlanItem): void }) {
  ...
  const extraCount = sameDay.length - 1;
  return <>
    <button type="button" className="vaccine-reminder-card collapsed info-summary-row" aria-label="查看疫苗安排" onClick={() => setOpen(true)}><span className="info-row-label">疫苗安排</span><span className="info-row-value">{next.vaccineName} · 第{next.dose}剂 · <span className={`vaccine-status ${status.tone}`}>{status.label}</span>{extraCount > 0 ? ` · 同日还有 ${extraCount} 针` : ''}</span><span className="info-row-chevron" aria-hidden="true">›</span></button>
    {open && <div className="modal-layer" onMouseDown={event => event.target === event.currentTarget && setOpen(false)}><section ref={dialogRef} className="editor info-sheet vaccine-info-sheet" role="dialog" aria-modal="true" aria-labelledby="next-vaccine-title"><header className="editor-head"><div><p className="kicker">疫苗安排 · {status.label}</p><h2 id="next-vaccine-title">{formatVaccineDay(reminderOn)}{sameDay.length > 1 ? ` · 共 ${sameDay.length} 针` : ''}</h2></div><button className="close-btn" onClick={() => setOpen(false)} aria-label="关闭">×</button></header><div className="vaccine-sheet-list">{sameDay.map(item => <div className="vaccine-sheet-item" key={item.key}><div className="vaccine-sheet-item-copy"><div className="vaccine-row-title"><strong>{item.vaccineName} · 第{item.dose}剂</strong><VaccineKind category={item.category} /></div>{item.record?.appointmentOn ? <small>门诊预约：{formatVaccineDay(item.record.appointmentOn)}{item.record.appointmentTime ? ` ${item.record.appointmentTime}` : ''}</small> : <small>建议接种：{formatVaccineDay(item.plannedOn)}前后</small>}</div><div className="vaccine-sheet-actions"><button type="button" className="btn secondary" onClick={() => { setOpen(false); onAppointment(item); }}>{item.record?.appointmentOn ? '改约' : '预约'}</button><button type="button" className="btn primary" onClick={() => { setOpen(false); onComplete(item); }}>记录</button></div></div>)}</div><p className="vaccine-safety-note">具体接种与补种安排请以接种门诊为准。</p></section></div>}
  </>;
}
