...
export function VaccineReminderCard({ profile, records, catalog, onComplete, onAppointment }: { profile: Profile; records: VaccineRecord[]; catalog: VaccineCatalogItem[]; onComplete(item: VaccinePlanItem): void; onAppointment(item: VaccinePlanItem): void }) {
  ...
  return <section className="vaccine-reminder-card" aria-labelledby="next-vaccine-title">
    <div className="vaccine-card-copy"><div className="vaccine-card-head"><p className="kicker">下一针疫苗 · {status.label}</p><button type="button" className="vaccine-collapse-button" aria-label="收起下一针疫苗" onClick={() => changeCollapsed(true)}><svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path d="M6 15l6-6 6 6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg></button></div><div className="vaccine-title-line"><h2 id="next-vaccine-title">{next.vaccineName} · 第{next.dose}剂</h2><VaccineKind category={next.category} /></div>{next.record?.appointmentOn ? <p><b>门诊预约：{formatVaccineDay(next.record.appointmentOn)}{next.record.appointmentTime ? ` ${next.record.appointmentTime}` : ''}</b><small>建议接种：{formatVaccineDay(next.plannedOn)}前后</small></p> : <p>建议接种：{formatVaccineDay(next.plannedOn)}前后</p>}</div>
    <div className="vaccine-card-actions"><button className="btn secondary" onClick={() => onAppointment(next)}>{next.record?.appointmentOn ? '修改预约' : '设置预约'}</button><button className="btn primary" onClick={() => onComplete(next)}>已接种</button></div>
  </section>;
}
...
export function VaccineEditor({ state, profile, catalog, records, onClose, onSave }: { state: VaccineEditorState; profile: Profile; catalog: VaccineCatalogItem[]; records: VaccineRecord[]; onClose(): void; onSave(value: DraftVaccineRecord): Promise<void> }) {
  ...
  function availableDoses(name: string) { const item = catalog.find(value => value.name === name); return Array.from({ length: item?.doseCount || 5 }, (_, index) => index + 1).filter(value => !genericAdd || !used.has(`${name}:${value}`)); }
  const selectableCatalog = genericAdd ? catalog.filter(item => item.active && availableDoses(item.name).length > 0) : catalog.map(item => item.name === (source?.vaccineName || state.item?.vaccineName) ? { ...item, active: true } : item);
  ...
  const [plannedOn, setPlannedOn] = useState(source?.plannedOn || state.item?.plannedOn || new Date().toISOString().slice(0, 10));
  const [administeredOn, setAdministeredOn] = useState(source?.administeredOn || (state.mode === 'complete' || state.mode === 'add' ? new Date().toISOString().slice(0, 10) : ''));
  ...
  const dirty = vaccineName !== (source?.vaccineName || state.item?.vaccineName || fallbackName) || dose !== String(initialDose) || plannedOn !== (source?.plannedOn || state.item?.plannedOn || new Date().toISOString().slice(0, 10)) || administeredOn !== (source?.administeredOn || (state.mode === 'complete' || state.mode === 'add' ? new Date().toISOString().slice(0, 10) : '')) || appointmentOn !== (source?.appointmentOn || '') || appointmentTime !== (source?.appointmentTime || '') || note !== (source?.note || '');
  ...
  const title = state.mode === 'appointment' ? (source?.appointmentOn ? '修改门诊预约' : '设置门诊预约') : source?.administeredOn ? '修改接种记录' : state.mode === 'complete' ? '确认已接种' : '添加接种记录';
  async function saveAppointment(clear = false) {
    setBusy(true); setError('');
    try { await onSave({ id: source?.id, vaccineName, category: state.item?.category || vaccineCategory(vaccineName), dose: Number(dose), plannedOn, appointmentOn: clear ? null : appointmentOn, appointmentTime: clear ? null : appointmentTime || null, administeredOn: null, note: source?.note || null }); onClose(); }
  ...
  return <div className="modal-layer" onMouseDown={event => event.target === event.currentTarget && !busy && requestClose()}><section ref={dialogRef} className="editor vaccine-editor" role="dialog" aria-modal="true" aria-labelledby="vaccine-editor-title"><header className="editor-head"><div><p className="kicker">疫苗记录</p><h2 id="vaccine-editor-title">{title}</h2></div><button className="close-btn" onClick={requestClose} aria-label="关闭">×</button></header><form className="editor-form" onSubmit={submit}>
    {state.mode === 'appointment' ? <><div className="vaccine-editor-summary"><b>{vaccineName} · 第{dose}剂</b><span>仅在门诊已经给出预约时填写。系统建议日期不会改变。</span></div><label>预约日期<input type="date" min={profile.birthDate} value={appointmentOn} onChange={event => setAppointmentOn(event.target.value)} required /></label><label>预约时间（选填）<input type="time" value={appointmentTime} onChange={event => setAppointmentTime(event.target.value)} /></label><p className="appointment-reference">建议接种：{formatVaccineDay(plannedOn)}前后</p></> : <>
      <label>接种日期<input type="date" min={profile.birthDate} max={new Date().toISOString().slice(0, 10)} value={administeredOn} onChange={event => setAdministeredOn(event.target.value)} required /></label>
    ...
    {error && <p className="error-text" role="alert">{error}</p>}<p className="vaccine-safety-note">接种时间仅用于家庭提醒，具体安排请以接种门诊为准。</p><footer className="editor-actions">{state.mode === 'appointment' && source?.appointmentOn ? <button type="button" className="btn danger-light" disabled={busy} onClick={() => void saveAppointment(true)}>取消预约</button> : <button type="button" className="btn secondary" onClick={requestClose}>取消</button>}<button className="btn primary" disabled={busy || (!groups.length && state.mode !== 'appointment')}>{busy ? '保存中…' : state.mode === 'complete' ? '确认完成' : '保存记录'}</button></footer>
  </form></section></div>;
}
...
export function VaccineHistory({ profile, records, catalog, deletedRecords, manager, onOpenEditor, onCancelAppointment, onDelete, onRestore, onLoadDeleted }: { profile: Profile; records: VaccineRecord[]; catalog: VaccineCatalogItem[]; deletedRecords: VaccineRecord[]; manager: boolean; onOpenEditor(state: VaccineEditorState): void; onCancelAppointment(item: VaccinePlanItem): void; onDelete(record: VaccineRecord): void; onRestore(record: VaccineRecord): void; onLoadDeleted(): void; }) {
  ...
  return <div className="vaccine-history"><section className="vaccine-overview"><div><span>已记录</span><strong>{completed.length}</strong><small>针</small></div><div><span>下一针</span><strong>{next ? formatVaccineDay(next.record?.appointmentOn || next.plannedOn).replace(/\d{4}年/, '') : '—'}</strong><small>{next ? `${next.record?.appointmentOn ? '已预约 · ' : ''}${next.vaccineName} · 第${next.dose}剂` : '当前计划已完成'}</small></div></section>
    {next && <section className="vaccine-next"><div className="vaccine-card-head"><p className="kicker">接下来</p></div><div className="vaccine-upcoming-list is-preview" aria-label="近期接种安排">{upcoming.slice(0, 3).map(item => <article key={item.key}><div><div className="vaccine-row-title"><strong>{item.vaccineName} · 第{item.dose}剂</strong><VaccineKind category={item.category} /></div><small>{item.record?.appointmentOn ? `已预约 · ${formatVaccineDay(item.record.appointmentOn)}` : `建议 ${formatVaccineDay(item.plannedOn)}前后`}</small></div><div className="upcoming-end"><span>{planStatus(item.record?.appointmentOn || item.plannedOn).label}</span><UpcomingVaccineActions item={item} onOpenEditor={onOpenEditor} onCancelAppointment={onCancelAppointment} /></div></article>)}</div>{upcoming.length > 3 && <button type="button" className="upcoming-toggle" onClick={openSchedule}>查看全部安排（{upcoming.length}）<span aria-hidden="true">›</span></button>}</section>}
