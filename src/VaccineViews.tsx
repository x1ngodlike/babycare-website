...
export function VaccineReminderCard({ profile, records, catalog, onComplete, onAppointment }: { profile: Profile; records: VaccineRecord[]; catalog: VaccineCatalogItem[]; onComplete(item: VaccinePlanItem): void; onAppointment(item: VaccinePlanItem): void }) {
  ...
  return <>
    <button type="button" className="vaccine-reminder-card collapsed info-summary-row" aria-label="查看疫苗安排" onClick={() => setOpen(true)}><span className="info-row-label">疫苗安排</span><span className="info-row-value">{next.vaccineName} · 第{next.dose}剂 · <span className={`vaccine-status ${status.tone}`}>{status.label}</span></span><span className="info-row-chevron" aria-hidden="true">›</span></button>
...
export function VaccineArchiveSummary({ profile, records, catalog, onOpen }: { profile: Profile; records: VaccineRecord[]; catalog: VaccineCatalogItem[]; onOpen(): void }) {
  ...
  return <section className="vaccine-archive-summary"><div className="section-title"><div><p className="kicker">健康记录</p><h2>疫苗记录</h2></div><span>{completed} 针已记录</span></div>{next ? <p>待接种：<b>{next.vaccineName} · 第{next.dose}剂</b><small>{vaccinePlanDateCopy(next)}</small></p> : <p>当前计划中的疫苗均已记录。</p>}<button className="btn secondary full" onClick={onOpen}>查看疫苗记录</button></section>;
}
