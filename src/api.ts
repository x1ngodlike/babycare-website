...
export const api = {
  ...
  restoreMemory: (id: string) => request<AiMemory>(`/api/ai/memories/${id}/restore`, { method: 'POST' }),
  ...
  createCareItem: (item: Pick<CareItem, 'name' | 'category' | 'icon' | 'sortOrder' | 'scheduleType' | 'intervalDays' | 'scheduleStartDate' | 'reminderTime' | 'reminderTimes' | 'scheduleEndDate' | 'weekDays' | 'patternDays'>) => request<CareItem>('/api/care-items', { method: 'POST', body: JSON.stringify(item) }),
  updateCareItem: (id: string, item: Pick<CareItem, 'name' | 'category' | 'icon' | 'sortOrder' | 'scheduleType' | 'intervalDays' | 'scheduleStartDate' | 'reminderTime' | 'reminderTimes' | 'scheduleEndDate' | 'weekDays' | 'patternDays'>) => request<CareItem>(`/api/care-items/${encodeURIComponent(id)}`, { method: 'PUT', body: JSON.stringify(item) }),
  ...
  careAdherence: () => request<{ items: { name: string; completionRate: number; completedDays: number; totalDays: number; streakDays: number; lastCompletedAt: string | null }[] }>('/api/care-items/adherence'),
