...
export const api = {
  ...
  createCareItem: (item: Pick<CareItem, 'name' | 'icon' | 'sortOrder' | 'scheduleType' | 'intervalDays' | 'scheduleStartDate' | 'reminderTime' | 'scheduleEndDate'>) => request<CareItem>('/api/care-items', { method: 'POST', body: JSON.stringify(item) }),
  updateCareItem: (id: string, item: Pick<CareItem, 'name' | 'icon' | 'sortOrder' | 'scheduleType' | 'intervalDays' | 'scheduleStartDate' | 'reminderTime' | 'scheduleEndDate'>) => request<CareItem>(`/api/care-items/${encodeURIComponent(id)}`, { method: 'PUT', body: JSON.stringify(item) }),
