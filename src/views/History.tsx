...
import { BASIC_SHAPES_ICON_PACK_ID, getBasicShapesRecordEmoji } from '../basicShapesIcons';
...
export function Timeline({ records, careItems, manager, emptyText = '这一天还没有记录', emptyAction, onEdit, onDelete, onAudit, iconForRecord, emojiForRecord, searchMode = false, compactMetadata = false, hideMetadata = false }: { records: CareRecord[]; careItems: CareItem[]; manager: boolean; emptyText?: string; emptyAction?: ReactNode; onEdit(record: CareRecord): void; onDelete(record: CareRecord): void; onAudit(record: CareRecord): void; iconForRecord?(record: CareRecord): string; emojiForRecord?(record: CareRecord): string | undefined; searchMode?: boolean; compactMetadata?: boolean; hideMetadata?: boolean }) {
  ...
  return <div className="timeline">{records.map(record => {
    ...
    const emoji = emojiForRecord?.(record);
    return <article className={`timeline-item ${record.type}${hasExtraNote ? ' has-note' : ''}`} key={record.id}><div className={`time-col${searchMode ? ' search-time' : ''}`}><time dateTime={record.occurredAt}>{searchMode && <span className="record-date">{date}</span>}<span>{time}</span></time><i /></div>{emoji ? <span className="record-mark emoji-icon" aria-hidden="true">{emoji}</span> : <img className="record-mark" src={iconForRecord?.(record) ?? careItemIcon(record, careItems)} alt="" />}<div className="record-copy"><strong><FeedingSummary record={record} careItems={careItems} /></strong>{compactMetadata && !hideMetadata ? <div className="record-meta-row"><small>{typeWithCreator}</small><em>{auditLabel}</em></div> : <small>{typeWithCreator}</small>}{record.note && <RecordNotePreview note={record.note} />}</div><ActionMenu label={`${summary(record, careItems)}的操作菜单`} items={items} /></article>;
  })}</div>;
}
...
export function HistoryView({ records, deletedRecords, vaccineRecords, vaccineCatalog, profile, iconPack, historyMode, setHistoryMode, careItems, manager, selected, setSelected, onEdit, onDelete, onAudit, onLoadDeleted, onRestore, onPurge, onOpenVaccineEditor, onCancelVaccineAppointment, onDeleteVaccine }: { records: CareRecord[]; deletedRecords: CareRecord[]; vaccineRecords: VaccineRecord[]; vaccineCatalog: VaccineCatalogItem[]; profile: Profile; iconPack: string; historyMode: 'care' | 'vaccine'; setHistoryMode(value: 'care' | 'vaccine'): void; careItems: CareItem[]; manager: boolean; selected: Date; setSelected(value: Date): void; onEdit(record: CareRecord): void; onDelete(record: CareRecord): void; onAudit(record: CareRecord): void; onLoadDeleted(): Promise<void>; onRestore(record: CareRecord): Promise<void>; onPurge(record: CareRecord): Promise<void>; onOpenVaccineEditor(state: VaccineEditorState): void; onCancelVaccineAppointment(item: VaccinePlanItem): void; onDeleteVaccine(record: VaccineRecord): void }) {
  ...
  return <div className="page-stack"><header className="page-head"><h1>历史记录</h1><p>{historyMode === 'care' ? '按日期查看，或搜索全部照护信息。' : '简单记录接种情况，及时查看下一针。'}</p></header>
    ...
    {historyMode === 'care' && view === 'active' && <SegmentedControl<'day' | 'overview'> className="record-view-tabs overview-toggle" label="查看方式" value={layout} options={[{ value: 'day', label: '按天' }, { value: 'overview', label: '七日' }]} onChange={setLayout} />}
    </div>
    ...
    view === 'deleted' ? <section className="deleted-records"><button className="inline-back" onClick={closeDeleted}>← 返回照护记录</button><div className="section-title"><h2>已删除记录</h2><span>{deletedRecords.length} 条</span></div>{deletedRecords.length ? deletedRecords.map(record => <article className="deleted-record" key={record.id}>{useBasicShapesEmoji ? <span className="record-mark emoji-icon" aria-hidden="true">{getBasicShapesRecordEmoji(record, careItems)}</span> : <img className="record-mark" src={themedRecordIcon(record)} alt="" />}<div><small>{new Date(record.occurredAt).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false })} · {record.type === 'supplement' && careItemCategory(record.supplement, careItems) === 'care' ? '护理' : typeNames[record.type]}</small><strong>{summary(record, careItems)}</strong><p>{auditNames[record.deletedBy || 'legacy']}删除 · {record.deletedAt ? new Date(record.deletedAt).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false }) : ''}</p></div><div className="deleted-actions"><button className="btn secondary" onClick={() => onRestore(record)}>恢复</button><button className="btn danger-button" onClick={() => onPurge(record)}>彻底删除</button></div></article>) : <EmptyState title="没有已删除记录" description="管理身份删除的记录会暂存在这里。" />}</section> : <>
    {toolbar}
    {layout === 'day' ? <>
