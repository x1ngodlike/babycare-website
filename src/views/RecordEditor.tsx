      ...
      <form className="editor-form" onSubmit={submit}>
        ...
        {value.type === 'feeding' && <div className="input-pair">
          <label>母乳量（mL）
            ...
            <div className="quick-values"><span>常用</span><div>{(prediction?.commonBreastValues?.length ? prediction.commonBreastValues : [30, 60, 90]).map(v => <button type="button" key={v} className="quick-value-btn" onClick={() => setValue({ ...value, breastMilkMl: v })}>{v}</button>)}</div></div>
          </label>
          <label>奶粉量（mL）
