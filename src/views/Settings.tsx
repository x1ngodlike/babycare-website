...
export default function SettingsView({ profile, careItems, vaccineCatalog, capabilities, user, pushStatus, theme, onThemeChange, heroTheme, onHeroThemeChange, heroThemeOverrides, onHeroThemeOverridesChange, onProfileSaved, onVaccineCatalogChanged, onCapabilitiesChanged, onCareItemsChanged, onImported, onLogout, onRefreshPush, onTestMorning, onTestFeedingGap, onTestCareItem, onSavePush }: { profile: Profile; careItems: CareItem[]; vaccineCatalog: VaccineCatalogItem[]; capabilities: Capabilities; user: SessionUser; pushStatus: PushStatus | null; theme: ThemeMode; onThemeChange(value: ThemeMode): void; heroTheme: string; onHeroThemeChange(value: string): void; heroThemeOverrides: Record<string, Partial<import('../config/weatherThemes').ThemeConfig>>; onHeroThemeOverridesChange(value: Record<string, Partial<import('../config/weatherThemes').ThemeConfig>>): void; onProfileSaved(value: Profile): void; onVaccineCatalogChanged(): Promise<void>; onCapabilitiesChanged(): Promise<void>; onCareItemsChanged(): Promise<void>; onImported(): void | Promise<void>; onLogout(): void; onRefreshPush(): Promise<void>; onTestMorning(): Promise<{ message: string }>; onTestFeedingGap(level: 'level1' | 'level2'): Promise<{ message: string }>; onTestCareItem(): Promise<{ message: string }>; onSavePush(data: PushSettingsPatch): Promise<PushStatus> }) {
  ...
  if (section !== 'root') return <div className="page-stack settings-subpage"><header className="subpage-head"><button type="button" onClick={back} aria-label="返回设置">←</button><div><p className="kicker">设置</p><h1>{subTitles[section]}</h1></div></header><div className="settings-grid">
    ...
    {section === 'appearance' && <AppearanceSettingsCard theme={theme} onChange={onThemeChange} heroTheme={heroTheme} onHeroThemeChange={onHeroThemeChange} heroThemeOverrides={heroThemeOverrides} onHeroThemeOverridesChange={onHeroThemeOverridesChange} profile={profile} userId={user.id} />}
