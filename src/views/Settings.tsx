...
import { Baby, Bell, Bot, ChevronRight, LogOut, Monitor, Pill, RefreshCw, Save, Send, Server, Syringe, Timer, Users } from 'lucide-react';
...
export default function SettingsView({ profile, careItems, vaccineCatalog, capabilities, user, pushStatus, theme, onThemeChange, heroTheme, onHeroThemeChange, randomThemeEnabled, randomThemeBackground, onRandomThemeEnabledChange, onRandomThemeShuffle, heroThemeOverrides, onHeroThemeOverridesChange, onProfileSaved, onVaccineCatalogChanged, onCapabilitiesChanged, onCareItemsChanged, onImported, onLogout, onRefreshPush, onTestMorning, onTestFeedingGap, onTestCareItem, onSavePush }: { profile: Profile; careItems: CareItem[]; vaccineCatalog: VaccineCatalogItem[]; capabilities: Capabilities; user: SessionUser; pushStatus: PushStatus | null; theme: ThemeMode; onThemeChange(value: ThemeMode): void; heroTheme: string; onHeroThemeChange(value: string): void; randomThemeEnabled: boolean; randomThemeBackground?: string; onRandomThemeEnabledChange(value: boolean): void; onRandomThemeShuffle(): void; heroThemeOverrides: Record<string, Partial<import('../config/weatherThemes').ThemeConfig>>; onHeroThemeOverridesChange(value: Record<string, Partial<import('../config/weatherThemes').ThemeConfig>>): void; onProfileSaved(value: Profile): void; onVaccineCatalogChanged(): Promise<void>; onCapabilitiesChanged(): Promise<void>; onCareItemsChanged(): Promise<void>; onImported(): void | Promise<void>; onLogout(): void; onRefreshPush(): Promise<void>; onTestMorning(): Promise<{ message: string }>; onTestFeedingGap(level: 'level1' | 'level2'): Promise<{ message: string }>; onTestCareItem(): Promise<{ message: string }>; onSavePush(data: PushSettingsPatch): Promise<PushStatus> }) {
  ...
  if (section !== 'root') return <div className="page-stack settings-subpage"><header className="subpage-head"><button type="button" onClick={back} aria-label="返回设置">←</button><div><p className="kicker">设置</p><h1>{subTitles[section]}</h1></div></header><div className="settings-grid">
    ...
    {section === 'appearance' && <AppearanceSettingsCard theme={theme} onChange={onThemeChange} heroTheme={heroTheme} onHeroThemeChange={onHeroThemeChange} randomThemeEnabled={randomThemeEnabled} randomThemeBackground={randomThemeBackground} onRandomThemeEnabledChange={onRandomThemeEnabledChange} onRandomThemeShuffle={onRandomThemeShuffle} heroThemeOverrides={heroThemeOverrides} onHeroThemeOverridesChange={onHeroThemeOverridesChange} profile={profile} userId={user.id} />}
    {section === 'feeding' && canManage(user) && <FeedingSettingsCard pushStatus={pushStatus} onSave={onSavePush} />}
  </div></div>;
  return <div className="page-stack settings-home"><header className="page-head"><h1>设置</h1><p>{user.role === 'superadmin' ? '管理家庭成员、照护项目和服务器。' : user.role === 'admin' ? '管理宝宝资料、用药项目和已删除记录。' : '查看当前身份和权限。'}</p></header><section className="account-card"><img src={member.icon} alt="" /><div><span>当前身份与权限</span><h2>{user.name}</h2><p>{roleNames[user.role]}</p></div><i className="status-badge brand">{canManage(user) ? '管理权限' : '记录权限'}</i></section>
