...
import { getIconPackAssets, getWeatherBackgroundAsset, getWeatherRecordIconByPack } from '../config/weatherThemes';
...
export function TodayView({ profile, records, recentRecords, vaccineRecords, vaccineCatalog, careItems, todayPlanStatus, capabilities, online, heroBg, iconPack, layout, weatherEffectsEnabled, onOpenSettings, onCompleteVaccine, onAppointmentVaccine, manager, superadmin, userId, weeklyGrowth, feedPrepEnabled, feedPrepMinutes, onAddGrowth, onAdd, onSupplement, onEdit, onDelete, onAudit }: { profile: Profile; records: CareRecord[]; recentRecords: CareRecord[]; vaccineRecords: VaccineRecord[]; vaccineCatalog: VaccineCatalogItem[]; careItems: CareItem[]; todayPlanStatus: 'loading' | 'ready' | 'error'; capabilities: Capabilities; online: boolean; heroBg: string; iconPack: string; layout: HeroLayout; weatherEffectsEnabled: boolean; onOpenSettings(): void; onCompleteVaccine(item: VaccinePlanItem): void; onAppointmentVaccine(item: VaccinePlanItem): void; manager: boolean; superadmin: boolean; userId: FamilyId; weeklyGrowth?: GrowthRecord; feedPrepEnabled: boolean; feedPrepMinutes: number; onAddGrowth(): void; onAdd(type: RecordType): void; onSupplement(value: Supplement): Promise<void>; onEdit(record: CareRecord): void; onDelete(record: CareRecord): void; onAudit(record: CareRecord): void }) {
  ...
  const diaryBackground = magazineTheme ? getWeatherBackgroundAsset(heroBg, diaryPeriod) : null;
  ...
  function renderMedicineTask(item: CareItem, timeLabel?: string) {
    ...
    return <article key={`medicine:${item.id}${timeLabel ? ':' + timeLabel : ''}`}>
      {useBasicShapesEmoji
        ? <span className={`task-icon emoji-icon${isOverdue ? ' overdue' : ''}`} aria-hidden="true">{BASIC_SHAPES_TASK_EMOJI[item.icon]}</span>
  ...
  function renderVaccineTask(item: VaccinePlanItem) {
    ...
    return <article className={`vaccine-task${overdue ? ' overdue' : ''}`} key={`vaccine:${item.key}`}>{useBasicShapesEmoji ? <span className="task-icon emoji-icon" aria-hidden="true">{BASIC_SHAPES_TASK_EMOJI.vaccine}</span> : <img className="task-icon vaccine" src={weatherThemeAssets?.tasks.vaccine ?? '/images/icons/tasks/vaccine.webp'} alt="" />}<div><b>{item.vaccineName} · 第{item.dose}剂</b><small>{statusLabel}</small></div><div className="today-plan-actions">{(!hasTodayAppointment || overdue) && <button className="btn secondary" aria-label={`${appointmentLabel}${item.vaccineName}第${item.dose}剂`} onClick={() => onAppointmentVaccine(item)}>{appointmentLabel}</button>}<button className="btn primary" aria-label={`记录${item.vaccineName}第${item.dose}剂已接种`} onClick={() => onCompleteVaccine(item)}>接种</button></div></article>;
  }
  return <div className="today-layout">
    ...
    <div className={`today-workbench${magazineTheme ? ' diary-workbench' : ''}`}>
      ...
      <section className="quick-section" aria-label="快捷记录"><div className="quick-grid"><button onClick={() => onAdd('feeding')}>{useBasicShapesEmoji ? <span className="quick-icon emoji-icon" aria-hidden="true">{BASIC_SHAPES_QUICK_EMOJI.feeding}</span> : <img className="quick-icon" src={quickIcons.feeding} alt="" />}<b>喂奶</b></button><button onClick={() => onAdd('bowel')}>{useBasicShapesEmoji ? <span className="quick-icon emoji-icon" aria-hidden="true">{BASIC_SHAPES_QUICK_EMOJI.bowel}</span> : <img className="quick-icon" src={quickIcons.bowel} alt="" />}<b>排便</b></button><button onClick={() => onAdd('supplement')}>{useBasicShapesEmoji ? <span className="quick-icon emoji-icon" aria-hidden="true">{BASIC_SHAPES_QUICK_EMOJI.care}</span> : <img className="quick-icon" src={quickIcons.care} alt="" />}<b>护理</b></button><button onClick={() => onAdd('note')}>{useBasicShapesEmoji ? <span className="quick-icon emoji-icon" aria-hidden="true">{BASIC_SHAPES_QUICK_EMOJI.note}</span> : <img className="quick-icon" src={quickIcons.note} alt="" />}<b>其他</b></button></div></section>
