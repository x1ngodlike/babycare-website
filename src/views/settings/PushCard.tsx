...
export type PushSettingsPatch = { enabled?: boolean; pushplusToken?: string; pushplusTopic?: string; morningDigestEnabled?: boolean; morningDigestTime?: string; feedingGapEnabled?: boolean; feedingGapLevel1Minutes?: number; feedingGapLevel2Minutes?: number; careItemEnabled?: boolean };
        ...
        <div className={`push-type-row${expanded.digest ? ' open' : ''}`}>
          <div className="push-type-head">
            <button type="button" className="push-type-toggle" aria-expanded={expanded.digest} onClick={() => toggleExpanded('digest')}>
              <b>早间日报</b><small>昨日汇总与今日计划，每天一条</small><i className="push-type-chevron" aria-hidden="true">›</i>
            </button>
            <span className={`push-type-badge ${pushStatus?.morningDigestEnabled ? (pushStatus.morningDigestTodaySent ? 'sent' : 'on') : ''}`}>{pushStatus?.morningDigestEnabled ? (pushStatus.morningDigestTodaySent ? '今日已发送' : '已开启') : '已关闭'}</span>
        ...
        <div className={`push-type-row${expanded.gap ? ' open' : ''}`}>
          <div className="push-type-head">
            <button type="button" className="push-type-toggle" aria-expanded={expanded.gap} onClick={() => toggleExpanded('gap')}>
              <b>喂奶间隔</b><small>超过轻度先提醒一次，超过重点再提醒一次</small><i className="push-type-chevron" aria-hidden="true">›</i>
            </button>
            <span className={`push-type-badge ${pushStatus?.feedingGapEnabled ? 'on' : ''}`}>{pushStatus?.feedingGapEnabled ? '已开启' : '已关闭'}</span>
        ...
        <div className={`push-type-row${expanded.care ? ' open' : ''}`}>
          <div className="push-type-head">
            <button type="button" className="push-type-toggle" aria-expanded={expanded.care} onClick={() => toggleExpanded('care')}>
              <b>用药护理</b><small>到点提醒吃药、推拿等定时照护</small><i className="push-type-chevron" aria-hidden="true">›</i>
            </button>
            <span className={`push-type-badge ${pushStatus?.careItemEnabled ? 'on' : ''}`}>{pushStatus?.careItemEnabled ? '已开启' : '已关闭'}</span>
    ...
    <section className="settings-card push-channels-card">
      ...
      <div className={`push-type-row push-channel-row${expanded.channel ? ' open' : ''}`}>
        <div className="push-type-head">
          <button type="button" className="push-type-toggle" aria-expanded={expanded.channel} onClick={() => toggleExpanded('channel')}>
            <b>微信推送</b><small>PushPlus 服务号消息，需配置 Token</small><i className="push-type-chevron" aria-hidden="true">›</i>
          </button>
          {pushStatus?.pushplusConfigured
            ? <span className={`push-type-badge ${pushStatus.enabled ? 'on' : ''}`}>{pushStatus.enabled ? '已开启' : '已关闭'}</span>
        ...
        {expanded.channel && <div className="push-type-body">
          <form className="push-channel-fields" onSubmit={event => { event.preventDefault(); void saveChannel(false); }}>
            ...
            <div className="push-channel-actions"><button type="submit" className="btn secondary" disabled={savingKey !== null || !channelDirty}>{savingKey === 'channel' ? '保存中…' : '保存配置'}</button>{pushStatus?.pushplusConfigured && <button type="button" className="btn secondary" disabled={savingKey !== null} onClick={() => void saveChannel(true)}>清除配置</button>}</div>
          </form>
      ...
      <div className="push-channel-row">
        <div className="push-channel-head">
          ...
          {onOpenAppNotifications ? <button type="button" className="btn secondary" onClick={onOpenAppNotifications}>本机设置</button> : <span className="push-channel-badge">仅 APP 内</span>}
        </div>
      </div>
    </section>
